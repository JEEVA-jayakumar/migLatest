...
//     return Vue.http.get("https://qaapp.bijlipay.co.in:8985/staticApi/plan-and-deivce-list-based-on-lead-source/" + request).then(response => {
//         let rental = [];
...
export const LEAD_AND_DEVICE_BASED_RENTAL1 = ({
    ...
    return Vue.http.get("https://qaapp.bijlipay.co.in:8985/staticApi/plan-and-deivce-list-based-on-lead-source/" + request.leadSource+"/"+request.device).then(response => {
        let rental = [];
        response.data.data.plan.map(function (value, key) {
            rental.push({
                label: value.planName,
...
export const LEAD_AND_DEVICE_BASED_RENTAL = ({
    ...
    // let leadSource = JSON.parse(request.leadSource);
    // let device = JSON.parse(request.device);
    return Vue.http.get("https://qaapp.bijlipay.co.in:8985/staticApi/plan-and-deivce-list-based-on-lead-source/" + request.leadSource.id+"/"+request.device.id).then(response => {
        let rental = [];
        response.data.data.plan.map(function (value, key) {
            rental.push({
                label: value.planName,
