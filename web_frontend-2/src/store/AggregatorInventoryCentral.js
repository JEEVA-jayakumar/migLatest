...
const InventoryCentral = {
  ...
  actions: {
    ...
    FETCH_AGGREGATORS_CENTRAL_INVENTORY_DASHBOARD_COUNT({ commit }, request) {
      ...
      return api
        .get("aggregator-inventory/agg-central-inventory-dashboard-device-count-list/"+request.id)
        .then(response => {
          // START=> COMMIT with data received'
          commit(
    ...
    FETCH_ALL_AGGREGATOR_INVENTORY_DEVICES_TYPES_DATA({ commit }, request) {
      return api.get("aggregator-inventory/agg-device").then(response => {
        // START=> COMMIT with data received'
        commit("SET_ALL_AGGREGATOR_INVENTORY_DEVICES_TYPES_DATA", response.data.data);
        // END=> COMMIT with data received'
      });
    },
