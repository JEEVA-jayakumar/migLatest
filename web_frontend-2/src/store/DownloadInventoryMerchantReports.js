...
const FileDownload1 = {
  ...
  actions: {
    async FETCH_INVENTORY_WITH_MERCHANT_ALL_DATAS({
      commit
    }, request) {
      var today = new Date();
      ...
      if (request.fromDate && request.toDate) {
        ...
        return await Vue.http
          .get("download-inventory-with-merchant-list/?from=" +
            request.fromDate +
            "&search=" +
            "&to=" + request.toDate, {
    ...
    // async FETCH_AGGREGATORS_INVENTORY_WITH_MERCHANT_ALL_DATAS({
    //   commit
    // }, request) {
    //   var today = new Date();
    //   var date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
    ...
    //     // console.log("==========", request)
    //     return await Vue.http
    //       .get("aggregator-inventory/agg-download-inventory-with-merchant-list/" + request.aggregators + "/" + "?from=" +
    //         request.fromDate +
    //         "&search=" +
    //         "&to=" + request.toDate, {
    //         responseType: 'arraybuffer'
    ...
    //     console.log("==========", request)
    //     return await Vue.http
    //       .get("aggregator-inventory/agg-download-inventory-with-merchant-list/" + request.aggregators, {
    //         responseType: 'arraybuffer'
    //       })
    //       .then(response => {
    //         console.log("FIle name : -- : ", response.headers["Content-Disposition"])
