              ...
              .catch(error => {
                ...
                if (error.data.hasOwnProperty("errorDetails")) {
                  ...
                  _.map(error.data.errorDetails, actual => {
                    ...
                    splitted.map(function(el) {
                      getErrorMessage += `.${el} `;
                    });
                    // $( `<p>Given: ${actual.value}</p> <p>Issue: ${actual.issue}</p>` ).insertAfter( `${getErrorMessage}` );

                    // $(getErrorMessage).append(`<p>Given: ${actual.value}</p> <p>Issue: ${actual.issue}</p>`);
                    $(".legalName").append("sad");
