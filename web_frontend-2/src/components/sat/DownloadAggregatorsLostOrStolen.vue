<template>
       ...
       <!-- @hide="emitfnshowAddPartner()"
        ...
        <form>
          ...
          <div class="col-md-12 group" align="right">
              ...
              >Cancel</q-btn>
              <q-btn align="right" @click="downloadLostDatas(formData)" :disabled="submitDisabled" color="purple-9">Download</q-btn>
            </div>
          </div>
        </form>
      </q-modal>
  ...
  export default {
    props: ["propLostStolenDatas"],
    data() {
      return {
        toggleModel: this.propLostStolenDatas,
    ...
    methods: {
         ...mapActions("DownloadLostOrStolenAllDatas",["FETCH_AGGREGATORS_LOST_OR_STOLEN_ALL_DATAS"]),
        emitfnshowLostOrStolen() {
        this.$emit("emitfnshowLostOrStolen");
       },
     
       downloadLostDatas(request) {
       let params = {
        ...
        this.FETCH_AGGREGATORS_LOST_OR_STOLEN_ALL_DATAS(params)
          .then(response => {
            this.$emit("emitfnshowLostOrStolen");
            this.$q.loading.hide();
            this.$q.notify({
