     ...
     <!-- @hide="emitfnshowAddPartner()"
      ...
      <form>
        ...
        <div class="col-md-12 group" align="right">
            ...
            >Cancel</q-btn>
            <q-btn align="right" @click="downloadLostData(formData)" :disabled="submitDisabled" color="purple-9">Download</q-btn>
          </div>
        </div>
      </form>
    </q-modal>
...
export default {
  props: ["propLostOrStolenReport"],
  data() {
    return {
      toggleModel: this.propLostOrStolenReport,
      tomorrow: addToDate(today, { days: 0 }),
      yesterday: subtractFromDate(today, { days: 180 }),
      state: new Date(),
  ...
  methods: {
    ...
    downloadLostData(request) {
     let params = {
      ...
      this.DOWNLOAD_lOST_OR_STOLEN_DATAS(params)
        .then(response => {
          this.$emit("emitfnshowLostOrStolen");
          this.$q.loading.hide();
          this.$q.notify({
