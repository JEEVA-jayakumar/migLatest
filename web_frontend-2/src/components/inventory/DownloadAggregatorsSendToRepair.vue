<template>
       ...
       <!-- @hide="emitfnshowAddPartner()"
        ...
        <form>
          <div class="column group">
            <div class="col-md-12">
              <div class="q-title text-weight-regular"><p align="center"><strong>Download Aggregator Lost Or Stolen Files</strong></p></div>
            </div>
            <div class="col-md-12">
          ...
          <div class="col-md-12 group" align="right">
              ...
              >Cancel</q-btn>
              <q-btn align="right" @click="downloadAggregatorsSendToRepair(formData)" :disabled="submitDisabled" color="purple-9">Download</q-btn>
            </div>
          </div>
        </form>
      </q-modal>
  ...
  export default {
    ...
    methods: {
         ...mapActions("DownloadInventorySendToRepair",["FETCH_SEND_TO_REPAIR_INVENTORY_ALL_DATAS", "FETCH_AGGREGATORS_SEND_TO_REPAIR_INVENTORY_ALL_DATAS"]),
        emitfnshowSendToRepair() {
        this.$emit("emitfnshowSendToRepair");
       },
     
       downloadAggregatorsSendToRepair(request) {
       let params = {
        ...
        this.FETCH_AGGREGATORS_SEND_TO_REPAIR_INVENTORY_ALL_DATAS(params)
          .then(response => {
            this.$emit("emitfnshowSendToRepair");
            this.$q.loading.hide();
            this.$q.notify({
          ...
          .catch(error => {
            ...
            if(error.status==400){
              ...
              color: "amber",
              position: "bottom",
              message:  "No Data Found Please Choose Some Another Date",
              icon: "thumb_down"
              });
            }
            else{
