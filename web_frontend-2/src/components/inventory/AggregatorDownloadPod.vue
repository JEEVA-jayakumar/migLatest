<template>
  ...
  export default {
    props: ["propPodListDatas","propData"],
    data() {
      return {
        toggleModel: this.propPodListDatas,
    ...
    methods: {
      ...mapActions("DownloadPodAllDatas", ["AGGREGATOR_POD_LISTED_ALL_DATAS"]),
      emitfnshowPodList() {
        this.$emit("emitfnshowPodList");
      },
  
      downloadPodList(request) {
        let params = {
        ...
        this.AGGREGATOR_POD_LISTED_ALL_DATAS(params)
          .then((response) => {
            this.$emit("emitfnshowPodList");
            this.$q.loading.hide();
            this.$q.notify({
