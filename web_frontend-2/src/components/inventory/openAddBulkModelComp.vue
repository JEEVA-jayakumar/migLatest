...
export default {
  ...
  methods: {
    ...
    // ]),
    ...mapActions("InventoryBulkUploadDevice", ["FEED_SEND_TO_REPAIR_DEVICE_BULK_UPLOAD_DATA","FEED_MOVIE_TO_SCRAP_DEVICE_BULK_UPLOAD_DATA"
    ]),
    deviceTypeOptions() {
      let cookedArr = [];
    ...
    moveToScrapUpload(formData){
      ...
      this.FEED_MOVIE_TO_SCRAP_DEVICE_BULK_UPLOAD_DATA(assumeFormDataValue)
        .then(response => {
          this.$q.notify({
            color: "positive",
            position: "bottom",
