                ...
                <div class="column group">
                    ...
                    <div class="col-md-12 group" align="right">
                        <q-btn flat align="right" class="bg-white text-weight-regular text-grey-8"
                            @click="emitfnshowEditRegion()">Cancel</q-btn>
...
export default {
    ...
    created() {
        ...
        this.formData.device = JSON.stringify(this.propRowDetails.device);
    ...
    methods: {
        ...
        ...mapActions("inventoryWithRegion", ["EDIT_INVENTORY_WITH_REGION_DETAILS"]),
        ...
        submitPodAndRegions(request) {
            ...
            finalRequest.propRowDatas.device = JSON.parse(request.device);
            ...
            this.EDIT_INVENTORY_WITH_REGION_DETAILS(params)
                .then((response) => {
                    this.$q.loading.show();
                    this.$q.notify({
                        color: "positive",
                    ...
                    // this.$router.push("/inventory/master/InventoryRegion");
                    this.FETCH_INVENTORY_WITH_REGION_DATAS();
                    this.emitfnshowEditRegion();
                })
                .catch((error) => {
                    this.$q.notify({
                        color: "negative",
        ...
        // LoadingDeviceAndRegionDatas() {
        //     this.formData.device = this.propRowDetails.device;
        //     this.formData.region = JSON.stringify(this.propRowDetails.region);
        // },
