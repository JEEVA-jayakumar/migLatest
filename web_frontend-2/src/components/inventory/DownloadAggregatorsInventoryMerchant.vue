<template>
  ...
  export default {
    props: ["propInventoryWithMerchant", "propData"],
    data() {
      return {
    ...
    methods: {
      ...mapActions("DownloadInventoryMerchantReports", [
        "FETCH_INVENTORY_WITH_MERCHANT_ALL_DATAS", "FETCH_AGGREGATORS_INVENTORY_WITH_MERCHANT_ALL_DATAS"
      ]),
      ...
      downloadInventoryWithMerchant(request) {
        ...
        this.FETCH_AGGREGATORS_INVENTORY_WITH_MERCHANT_ALL_DATAS(params)
          .then((response) => {
            this.$emit("emitfnshowInventoryWithMerchant");
            this.$q.loading.hide();
            this.$q.notify({
