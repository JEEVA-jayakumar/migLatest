...
export default {
  ...
  methods: {
    ...
    ...mapActions("sparePartsInventory", [
      "CREATE_SPARE_POD_LIST_INVENTORY", "CREATE_PHONEPE_SPARE_POD_LIST_INVENTORY"
    ]),
    finding(request) {
    ...
    fnSubmitBankDetails(request) {
      ...
      if (this.$v.flag.$error) {
        ...
        // for((SpareDetails) in this.formData.sparePartsSets){
        //     if(SpareDetails.spareParts.id ==this.formData.sparePartsSets[0].spareParts.id){
        //       this.formData.sparePartsSets = SpareDetails.spareParts
        //     }
        //     console.log("SpareDetails ------->",this.formData.sparePartsSets)
        // };
        this.$q.loading.show();
        this.CREATE_PHONEPE_SPARE_POD_LIST_INVENTORY(request)
          .then(() => {
            this.$q.loading.hide();
            this.$q.notify({
              color: "positive",
