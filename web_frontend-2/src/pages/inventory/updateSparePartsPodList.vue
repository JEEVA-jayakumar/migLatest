...
export default {
  ...
  methods: {
    ...
    fnSubmitBankDetails(request) {
      ...
      param.status = this.getInventoryUpdatedPodDetails.status == '6'?  2 :this.getInventoryUpdatedPodDetails.status ;
      ...
      this.$q.loading.show();
      this.SPARE_PARTS_POD_UPDATE_DETAILS(param)
        .then(() => {
          this.$q.loading.hide();
          this.$q.notify({
            color: "positive",
          ...
          this.$router.push("/inventory/master/spareParts");
        })
        .catch(error => {
          this.$q.loading.hide();
          this.$q.notify({
