        ...
        <div class="col-md-7">
          ...
          />
                 ...
                 <div class="col-md-2">
                    ...
                    <div v-for="menu in sparePartsTypes" :key="menu.id" :to="menu.to">
                        <div :id="menu.spareParts.id">
                            ...
                            <input :id="menu.spareParts.id" :value="menu.count" type="number" min="1" max="5000"
                                @click="finding(menu.count)" @blur="getAllCounts($event, menu)" />
                        </div>
                    </div>
                </div>
                <div class="col-md-2">
            ...
            <div class="full-width group" align="center">
                <q-btn size="md" type="button" color="purple-9" @click="fnSubmitBankDetails(formData)">Update</q-btn>
            </div>
        </form>
    </div>
</template>
...
export default {
    ...
    beforeMount() {
        this.sparePartsTypes = this.getPhonepeInventoryUpdatedPodDetails.sparePartsSets;
        this.formData.sparePartsSets = this.getPhonepeInventoryUpdatedPodDetails.sparePartsSets;
    ...
    mounted() {
        ...
        (this.formData.allocate_region = JSON.stringify(
            ...
            (this.formData.allocate_so != null ? JSON.stringify(this.propRowDetails.allocate_so) : null);
    ...
    methods: {
        ...
        ...mapActions("sparePodUpdateDetails", [
            "SPARE_PARTS_POD_UPDATE_DETAILS", "PHONEPE_SPARE_PARTS_POD_UPDATE_DETAILS",
        ...
        fnSubmitBankDetails(request) {
            ...
            console.log("request.allocate_so------->", request.allocate_so != "" ? JSON.parse(request.allocate_so) : null);
            let param = {
                allocate_region: JSON.parse(request.allocate_region),
                allocate_so: request.allocate_so != "" ? JSON.parse(request.allocate_so) : null,
                allocate_reseller: request.allocate_reseller,
                pod_number: request.pod_number,
                total_count: request.total_count,
                sparePartsSets: request.sparePartsSets
            };
            param.created_by = this.getPhonepeInventoryUpdatedPodDetails.created_by;
            param.status = this.getPhonepeInventoryUpdatedPodDetails.status == '6' ? 2 : this.getPhonepeInventoryUpdatedPodDetails.status;
            param.owner = this.getPhonepeInventoryUpdatedPodDetails.owner;
            param.created_date = this.getPhonepeInventoryUpdatedPodDetails.created_date;


            console.log(
                "getPhonepeInventoryUpdatedPodDetails PARAM VALUES GETTED----------------->",
                JSON.stringify(this.getPhonepeInventoryUpdatedPodDetails)
            );
            this.$q.loading.show();
            this.PHONEPE_SPARE_PARTS_POD_UPDATE_DETAILS(param)
                .then(() => {
                    this.$q.loading.hide();
                    this.$q.notify({
                        color: "positive",
                    ...
                    this.$router.push({name: "AggregatorsspareParts"});
                })
                .catch(error => {
                    this.$q.loading.hide();
                    this.$q.notify({
