    ...
    <div>
      ...
      >
        ...
        <span class="label text-positive" v-if="props.row.status == 1"
            >Created</span
          >
          <span class="label text-primary" v-else-if="props.row.status == 2"
...
export default {
  ...
  methods: {
    ...
    ...mapActions("sparePartsPodListInventoryDetails", [
      "FETCH_SPARE_PARTS_POD_LIST_INVENTORY_DETAILS"
    ]),
      ...
      this.$q
        ...
        .then(() => {
          ...
          this.FETCH_INVENTORY_UPDATED_POD_DETAILS(param).then(() => {
            ...
            console.log(
              "the  tableData1 data " + JSON.stringify(this.tableData1)
            );
            this.$q.loading.hide();
            this.$router.push({
    ...
    ajaxLoadAllLeadInfo({ pagination, filter }) {
      ...
      this.FETCH_SPARE_PARTS_POD_LIST_INVENTORY_DETAILS({ pagination, filter })
        .then(res => {
          ...
          this.paginationControl.rowsNumber = this.getsparePartsPodListDetails.totalElements;
          this.paginationControl.page =
            this.getsparePartsPodListDetails.number + 1;

          // then we update the rows with the fetched ones
          this.tableData = this.getsparePartsPodListDetails.content;
          if (this.getsparePartsPodListDetails.sort != null) {
            this.paginationControl.sortBy = this.getsparePartsPodListDetails.sort[0].property;
            this.paginationControl.descending = this.getsparePartsPodListDetails.sort[0].ascending;
          }
