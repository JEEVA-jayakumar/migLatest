...
export default {
  ...
  beforeMount() {
    console.log(
      ...
      this.formData.tokenDeviceModel
    );
  },
  ...
  methods: {
    ...mapActions("MarsApiSync", ["DEVICE_MODEL_MARS"]),
    ...
    fnFinalSubmit(request) {
      ...
      if (this.$v.formData.$error) {
        ...
        // let requestParams = {
        //   token: this.formData.tokenDeviceModel
        // };
        this.$q.loading.show({
          delay: 0, // ms
        ...
        this.DEVICE_MODEL_MARS(this.formData.tokenDeviceModel)
          .then(response => {
            console.log("fnFinalSubmit-------------->>>>",JSON.stringify(response))
            this.$q.loading.hide();
            this.$q.notify({
              color: "positive",
              position: "bottom",
