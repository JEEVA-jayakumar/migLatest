      ...
      <q-pull-to-refresh :distance="30" :handler="PullToRefresh" inline>
        ...
        >
          ...
          <template v-slot:body-cell-checkStatus="props">
            <q-td :props="props" v-if="props.row">
              <q-btn
                v-if="props.row.leadStatus == 6 || props.row.leadStatus == 9"
                push
                color="green"
                size="sm"
                @click="checkStatus(props.row)"
              >Check Status</q-btn>
            </q-td>
          </template>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters("SatLeadValidation", ["getCallbackResponse"])
  },
  mounted() {
  ...
  methods: {
    ...
    ...mapActions("SatLeadValidation", ["FETCH_CALL_BACK_RES"]),
    ...
    checkStatus(data) {
      ...
      this.FETCH_CALL_BACK_RES(data)
        .then(() => {
          this.$q.loading.hide();
          if (this.getCallbackResponse.status == "00") {
            this.$q.notify({
              color: "positive",
              position: "bottom",
              message: this.getCallbackResponse.remarks,
              icon: "thumb_up"
            });
            this.ajaxLoadAllLeadInfo({
              pagination: this.paginationControl,
