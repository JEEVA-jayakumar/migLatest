  ...
  <q-page padding>
    ...
    ><q-step default name="campaign" title="Edit Rental Details">
      ...
      <q-step name="ad_group" title="Edit KYC Fields">
        <p class="q-caption"><strong>Edit KYC Details</strong></p>
        <div class="row q-col-gutter-sm">
            ...
            <div class="col-xs-12 col-sm-6">
              <q-input :model-value="merchant.companyinformation.pan" @update:model-value="val => { merchant.companyinformation.pan = val ? val.toUpperCase() : val; }"
                  color="grey-9" @blur="v$.merchant?.companyinformation?.pan?.$touch" :error="v$.merchant?.companyinformation?.pan?.$error" class="no-margin" label="Pan Number"/>
              <div
                  class="text-negative"
...
export default {
     ...
     this.CMS_EDIT_MERCHANT(requestparams)
        ...
        .catch(error => {
          ...
          this.$q.notify({
            ...
            position: "bottom",
            icon: "thumb_down",
            message: error.response?.data?.errorDetails[1].field +"-" +error.response?.data?.errorDetails[1].issue
          });}
          else{
                 this.$q.notify({
            color: "negative",
            position: "bottom",
            icon: "thumb_down",
            message: "Merchant Details" +error.response?.data?.message
          });
          }
      ...
      self
        ...
        .then(() => {
         ...
         self.cityFromMars.items.map(oo => {
            ...
            self.originalCityOptions = self.cityOptions;
        }).then(() => {
          ...
          return self.STATE_FROM_MARS().then(response => {
            ...
            self.originalStateOptions = self.stateOptions;
  ...
  this.FETCH_SHORT_LEAD_DATA(this.getImplementedQueue.leadInformation.id)
    ...
    /* IFSC bank search result */
       residentCitySearch(val, update) {
      update(() => {
        this.cityOptions = this.COMMON_FILTER_FUNCTION(this.originalCityOptions, val);
      });
    },
      ...
      residentStateSearch(val, update) {
      update(() => {
        this.stateOptions = this.COMMON_FILTER_FUNCTION(this.originalStateOptions, val);
      });
    },
