        ...
        .onOk(() => {
          ...
          this.VERIFY_LEAD_DOCUMENTS({ id: this.$route.params.id, params: [] })
            ...
            .catch(error => {
              ...
              if (errData?.data?.toBeVerifiedDocuments?.length > 0) {
                let arrayMessage = "";
                _.map(errData.data.toBeVerifiedDocuments, oo => {
                  arrayMessage += `${oo}, `;
                });
                this.$q.notify({
                  color: "amber-9",
                ...
                if (errData?.data) delete errData.data.toBeVerifiedDocuments;
                for (var key in errData?.data) {
                  let arrayMessage = "";
                  _.map(errData.data[key], oo => {
                    arrayMessage += `${oo}, `;
                  });
                  this.$q.notify({
                    color: "amber-9",
                    position: "bottom-left",
                    message: `${errData.message} for key`,
                    detail: arrayMessage,
                    timeout: 8000,
                    icon: "warning",
                    actions: [
