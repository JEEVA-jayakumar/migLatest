...
export default {
  ...
  this.FETCH_SHORT_LEAD_DATA(this.getImplementedQueue.leadInformation.id)
    ...
    fnResidentStateSearch(val, update) {
      if (val.length < 1) { update(() => { this.stateOptionsSearch = []; }); return; }
      update(() => { this.stateOptionsSearch = this.COMMON_FILTER_FUNCTION(this.stateOptions, val); });
    },
    fnResidentCitySearch(val, update) {
      if (val.length < 3) { update(() => { this.cityOptionsSearch = []; }); return; }
      update(() => { this.cityOptionsSearch = this.COMMON_FILTER_FUNCTION(this.cityOptions, val); });
    },
