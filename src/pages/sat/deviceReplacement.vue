    ...
    <div>
      ...
      <q-card class="group q-pa-md" v-if="selectedTab == 'assigned'">
        <div class="row items-center q-col-gutter-y-sm">
          ...
          <div class="col-md-3 col-sm-6 col-xs-6">
            <q-select use-input input-debounce="0" @filter="fnFilterAssignToReassign" clearable
              v-model="formData.assignToReassign" :options="assignToOptionsReassignFiltered"
              label="Re-Assign To" color="grey-9" :disable="isReAssignDropdownDisabled"
              emit-value map-options @click="handleReAssignDropdownClick" />
          </div>
          <div class="col-md-3 col-sm-6 col-xs-6" align="right">
            <div class="column items-end">
              <q-btn no-caps label="Re-Assign" class="common-dark-blue q-mb-sm"
                :disabled="isReAssignDisabled" :loading="reAssignLoading" @click="reAssignImplementationUser" />
              <q-btn no-caps label="Un-Assign" class="common-dark-blue q-mb-sm"
                :disabled="isUnAssignDisabled" :loading="unAssignLoading" @click="unAssignImplementationUser" />
            </div>
          </div>
        </div>
      </q-card>
      ...
      <q-tab-panels v-model="selectedTab" animated>
            ...
            <template v-slot:body-cell-leadNumber="props">
              <q-td :props="props" class="cursor-pointer" @click="toggleLeadInformation(props.row.leadInformation)">
                <span class="label text-primary">#
                  {{ props.row.leadInformation == null ? props.row.qrLeadInformation.qrLeadNumber : props.row.leadInformation.leadNumber }}
                </span>
              </q-td>
            </template>
        ...
        <q-tab-panel name="unAssigned" class="no-padding">
          <q-table :rows="tableData1" :columns="columnDataUnassigned" table-class="customTableClass"
            ...
            <template v-slot:body-cell-tid="props">
              <q-td :props="props" class="customTd">
                <div class="text-primary">{{ props.row.tid == null ? "NA" : props.row.tid }}</div>
              </q-td>
            </template>
            <template v-slot:body-cell-mid="props">
              <q-td :props="props" class="customTd">
...
export default {
  ...
  data() {
    return {
      ...
      columnDataAssigned: [
        ...
        { name: "merchant_name", required: true, label: "Merchant Name", align: "left", field: row => row.leadInformation != null && row.leadInformation.leadName != null ? row.leadInformation.leadName : row.qrLeadInformation != null && row.qrLeadInformation.legalName != null ? row.qrLeadInformation.legalName : "NA", sortable: false },
        ...
        { name: "assigned_to", required: true, label: "Assigned To", align: "left", field: row => row.replacedBy == null ? "NA" : row.replacedBy.name, sortable: false },
        ...
        { name: "submitToMarsDate", required: true, label: "Date of Submission", align: "left", field: row => row.leadInformation == null || row.leadInformation.submitToMarsDate == null ? "NA" : row.leadInformation.submitToMarsDate, sortable: true },
      ],
  ...
  methods: {
    ...
    handleReAssignDropdownClick() {
      if (this.isReAssignDropdownDisabled) {
        let message = this.selectedAssignedCount > 1 ? 'Multiple selection not allowed' : 'Select at least one device to re-assign';
        this.$q.notify({ color: "warning", position: "bottom", message, icon: "info" });
      }
    },
    ...
    unAssignImplementationUser() {
      ...
      let postValues = { action: this.$MARS_DEVICE_STATUS_TID_GENERATED, marsDeviceIds, userId: this.formData.assignToReassign };
      this.DEVICE_REPLACEMENT_QUEUE_UNASSIGN(postValues)
        ...
        .catch(() => { this.$q.notify({ color: "negative", position: "bottom", message: "Please try again", icon: "thumb_down" }); })
    ...
    reAssignImplementationUser() {
      ...
      let marsDeviceIds = this.formData.marsDeviceIdsCookedUnAssinged.map(v => v.id);
      let postValues = { action: this.$MARS_DEVICE_STATUS_SAT_ASSIGNED, marsDeviceIds, userId: this.formData.assignToReassign };
  ...
  watch: {
    'formData.marsDeviceIdsCookedUnAssinged'() {
      if (this.selectedAssignedCount !== 1) { this.formData.assignToReassign = ''; }
    },
  },
};
</script>
