  ...
  <q-page>
    <div>
      ...
      <q-tab-panels v-model="selectedTab" animated>
        <q-tab-panel name="allocatedSo" class="q-pa-none">
          <allocatedSo />
        </q-tab-panel>
            ...
            <template v-slot:body-cell-action="props">
              <q-td :props="props">
                <div class="row no-wrap no-padding">
                  <q-btn dense no-caps no-wrap label="Reject" icon="far fa-minus-square" size="sm"
                    @click="fnRejectPodDetails(props.row)" flat class="text-negative" />
                  <q-btn dense no-caps no-wrap label="Approve" icon="far fa-plus-square" size="sm"
                    @click="fnApprovePodDetails(props.row)" flat class="text-light-blue" />
                </div>
              </q-td>
            </template>
            <template v-slot:top>
              <div class="col-md-5">
                <q-input dense clearable color="grey-9" v-model="filterSearch"
                  label="Search By POD Number" placeholder="Type.." class="q-mr-lg q-py-sm">
        ...
        <q-tab-panel name="stocks" class="q-pa-none">
          ...
          >
            ...
            <template v-slot:top>
              <div class="col-md-5">
                <q-input dense clearable color="grey-9" v-model="filterSearch1"
                  label="Search By POD Number" placeholder="Type.." class="q-mr-lg q-py-sm">
                  <template v-slot:append><q-icon name="search" /></template>
                </q-input>
              </div>
        ...
        <q-tab-panel name="createSoPod">
          <div class="col-sm-3 q-pa-md">
            ...
            <div class="col-md-7 q-mt-md">
              ...
              <div class="row">
                <div class="row items-center q-mr-md" v-for="menu in sparePartsTypes" :key="menu.value.id">
                  <q-checkbox v-model="menu.checked" :label="menu.label"
              ...
              <div v-for="menu in sparePartsTypes" :key="'dv' + menu.value.id">
                <div v-if="menu.checked" class="q-mt-sm">
                  <q-input type="number" v-model.number="menu.count" :label="menu.label + ' Count'"
                    style="max-width: 200px" @blur="getAllCounts(menu)" />
                </div>
              </div>
            </div>
            ...
            <div class="full-width q-mt-md" align="center">
              <q-btn
                :disabled="formData.total_count == 0 || !formData.allocate_so"
                size="md" label="Submit" color="purple-9"
...
export default {
  ...
  data() {
    return {
      ...
      paginationControl: { rowsNumber: 0, page: 1, sortBy: "updated_date", descending: true, rowsPerPage: 5 },
      paginationControl1: { rowsNumber: 0, page: 1, sortBy: "updated_date", descending: true, rowsPerPage: 5 },
      columns1: [
        { name: "pod_number", required: true, label: "Pod Number", align: "left", field: "pod_number", sortable: true },
        { name: "regionAreaName", required: true, label: "Region", align: "left", field: row => row.allocate_region ? row.allocate_region.regionAreaName : "NA" },
  ...
  methods: {
    ...
    filterRegionBasedSo(val, update) {
      if (val === "") {
        update(() => { this.regionBasedSoFiltered = this.regionBasedSo; });
        return;
      }
      update(() => {
        const needle = val.toLowerCase();
        this.regionBasedSoFiltered = this.regionBasedSo.filter(v => v.label.toLowerCase().indexOf(needle) > -1);
      });
    },
    fnRejectPodDetails(exceptionDetails) {
      this.showRejectModel = !this.showRejectModel;
      if (exceptionDetails) { this.propsRejectAppend = exceptionDetails; }
    },
    podNumberCreation() {
      const name = "SO";
      const d = new Date();
    ...
    fnregionSpareCount() {
      ...
      this.FETCH_SPARE_PARTS_SETS_AND_COUNTS().then(() => {
        this.getsparePartsSetsAndCounts.forEach(item => {
          this.regionSpareCount.push({ value: item, label: item.spare_parts_types });
        });
      });
    },
    fnApprovePodDetails(reqdata) {
      this.$q.dialog({ title: "Confirm", message: "Are you sure want to Approve?", ok: "Continue", cancel: "Cancel" })
        .onOk(() => {
          this.$q.loading.show();
          this.FETCH_SINGLE_POD_DETAILS(reqdata).then(response => {
            let param = {
    ...
    ajaxLoadAllLeadInfo({ pagination, filter }) {
      ...
      this.FETCHING_INCOMING_POD_LIST_DETAILS({ pagination, filter })
        .then(res => {
          this.paginationControl = pagination;
          this.paginationControl.rowsNumber = this.getregionalInventoryPodDetails.totalElements;
          this.paginationControl.page = this.getregionalInventoryPodDetails.number + 1;
          this.tableData = this.getregionalInventoryPodDetails.content;
          if (this.getregionalInventoryPodDetails.sort != null) {
            this.paginationControl.sortBy = this.getregionalInventoryPodDetails.sort[0].property;
            this.paginationControl.descending = this.getregionalInventoryPodDetails.sort[0].ascending;
          }
          this.$q.loading.hide();
        })
    ...
    ajaxLoadAllLeadInfo1({ pagination, filter }) {
      this.$q.loading.show({ delay: 0, spinnerColor: "purple-9", message: "Fetching data .." });
      this.FETCHING_INCOMING_STOCKS_POD_LIST_DETAILS({ pagination, filter })
        .then(res => {
          this.paginationControl1 = pagination;
          this.paginationControl1.rowsNumber = this.getregionalInventoryStocksPodDetails.totalElements;
          this.paginationControl1.page = this.getregionalInventoryStocksPodDetails.number + 1;
          this.tableData1 = this.getregionalInventoryStocksPodDetails.content;
          if (this.getregionalInventoryStocksPodDetails.sort != null) {
            this.paginationControl1.sortBy = this.getregionalInventoryStocksPodDetails.sort[0].property;
            this.paginationControl1.descending = this.getregionalInventoryStocksPodDetails.sort[0].ascending;
          }
          this.$q.loading.hide();
        })
        .catch(() => this.$q.loading.hide());
    ...
    fnSubmitBankDetails(request) {
      ...
      this.$q.loading.show();
      this.UPDATE_SPARE_PARTS_INVENTORY_DATAS(payload)
        .then(() => {
          this.$q.loading.hide();
          ...
          this.sparePartsTypes.forEach(t => { t.checked = false; t.count = 0; });
          this.podNumberCreation();
        })
        .catch(error => {
          this.$q.loading.hide();
    ...
    fnSparePartsTypes() {
      this.FETCH_SPARE_PARTS_GET_TYPES().then(() => {
        this.sparePartsTypes = this.getsparePartsGetTypes.map(item => ({
          value: item, label: item.spare_parts_types, checked: false, count: 0
        }));
      });
    },
    ...
    getAllCounts(menu) {
      ...
      else { this.formData.sparePartsSets.push({ count: val, spareParts: { id: menu.value.id } }); }
      this.calculateTotalCount();
    },
    calculateTotalCount() {
