  ...
  actions: {
    ...
    async FETCH_PHONE_PE_INVENTORY_DEVICE_HISTORY_DATAS({
      ...
      if (request.from && request.to) {
        // console.log("==========", request)
        return await api
          .get("aggregator-inventory/agg-download-device-history/"+getAggregatorId()+"?from=" + request.from + "&to=" + request.to, {
            responseType: 'arraybuffer'
          }).then(response => {
            console.log("FIle name : -- : ", response)
            let blob = COMMON_FILE_DOWNLOAD(response);
        ...
        console.log("==========", request)
        return await api
          .get("aggregator-inventory/agg-download-device-history/"+getAggregatorId(), {
            responseType: 'arraybuffer'
          }).then(response => {
            console.log("FIle name : -- : ", response.headers["Content-Disposition"])
            let blob = COMMON_FILE_DOWNLOAD(response);
