...
function getNIIFromStorage(key) {
  const raw = localStorage.getItem(key);
  if (!raw) return '';
  let str = raw;
  try { str = JSON.parse(raw); } catch (e) { str = raw; }
  const idx = String(str).lastIndexOf('|');
  return idx >= 0 ? String(str).substring(idx + 1) : String(str);
}
...
export const MARS_DATA_SUBMIT_EXTERNAL = ({
  ...
  let res = getNIIFromStorage("aa_t");
  console.log("Test NII:", res);
  let headers = {
    'NII': res
  };
...
export const MARS_STATIC_QR_DATA_SUBMIT = ({
  ...
  let res = getNIIFromStorage("bb_t");
  let headers = {
    'NII': res
  };
