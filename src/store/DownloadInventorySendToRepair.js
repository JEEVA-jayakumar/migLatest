...
const FileDownload1 = {
  ...
  actions: {
    async FETCH_SEND_TO_REPAIR_INVENTORY_ALL_DATAS({
      commit
    }, request) {
      var today = new Date();
    ...
    // async FETCH_AGGREGATORS_SEND_TO_REPAIR_INVENTORY_ALL_DATAS({
    //   commit
    // }, request) {
    //   var today = new Date();
    //   var date = today.getFullYear() + '-' + (today.getMonth() + 1) + '-' + today.getDate();
    ...
    //     // console.log("==========", request)
    //     return await api
    //       .get("aggregator-inventory/agg-download-central-inventory-device-by-status/3/" + request.aggregators + "?from=" + request.from + "&to=" + request.to, {
    //         responseType: 'arraybuffer'
    //       })
    //       .then(response => {
    //         console.log("FIle name : -- : ", response)
    ...
    //     console.log("==========", request)
    //     return await api
    //       .get("aggregator-inventory/agg-download-central-inventory-device-by-status/3/" + request.aggregators, {
    //         responseType: 'arraybuffer'
    //       })
    //       .then(response => {
    //         console.log("FIle name : -- : ", response.headers["Content-Disposition"])
    ...
    async FETCH_AGGREGATORS_SEND_TO_REPAIR_INVENTORY_ALL_DATAS({
      ...
      if (request.from && request.to) {
        // console.log("==========", request)
        return await api
          .get("aggregator-inventory/agg-download-central-inventory-device-by-status/3/"+getAggregatorId()+ "?from=" + request.from + "&to=" + request.to, {
            responseType: 'arraybuffer'
          }).then(response => {
            console.log("FIle name : -- : ", response)
            let blob = COMMON_FILE_DOWNLOAD(response);
        ...
        console.log("==========", request)
        return await api
          .get("aggregator-inventory/agg-download-central-inventory-device-by-status/3/"+getAggregatorId(), {
            responseType: 'arraybuffer'
          }).then(response => {
            console.log("FIle name : -- : ", response.headers["Content-Disposition"])
            let blob = COMMON_FILE_DOWNLOAD(response);
