  ...
  actions: {
    ...
    async AGGREGATOR_POD_LISTED_ALL_DATAS({
      ...
      if (request.fromDate && request.toDate) {
        // console.log("==========", request)
        return await api
          .get("aggregator-inventory/agg-download-pod-details/" + request.fromDate + "/" + request.toDate + "/"+getAggregatorId(), {
            responseType: 'arraybuffer'
          }).then(response => {
            console.log("FIle name : -- : ", response.headers)
            console.log("FIle name : -- : ", response["Access-Control-Expose-Headers"])
        ...
        console.log("==========", request)
        return await api
          .get("aggregator-inventory/agg-download-pod-details/"+getAggregatorId(), {
            responseType: 'arraybuffer'
          }).then(response => {
            console.log("FIle name : -- : ", response)
            let blob = COMMON_FILE_DOWNLOAD(response);
